...
`click-to-walk.spec.cjs` completes the intro, clicks a known walkable Library Foyer cell through the canvas, and verifies that the canonical player position changes without a page error. It also proves that a stale red no-path cursor/hover cell cannot poison the next click target, and that every player and NPC movement frame is decoded in the shared cache before play.
