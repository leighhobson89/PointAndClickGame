...
| BUG-022 | P3 | resolved | Executable canvas access now uses an explicit local variable, function parameter, or `getElements().canvas`; the edge-scroll named-global dependency was removed. | Dependency/static inspection and navigation/dialogue browser suites pass without a named-global dependency. |
